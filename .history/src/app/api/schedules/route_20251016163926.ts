...
import { cloudTasksService } from '@/lib/cloud-tasks';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (newSchedule) {
      // Créer une Cloud Task pour la planification
      try {
        const taskName = await cloudTasksService.schedulePublishTask(
          scheduleId,
          videoId,
          accountId,
          userId,
        ...
        // Mettre à jour la planification avec le nom de la tâche
        await scheduleService.update(scheduleId, {
          // Ajouter le taskName si nécessaire
        });

        console.log(`Tâche Cloud Tasks créée: ${taskName}`);
      } catch (taskError) {
        console.error('Erreur lors de la création de la tâche Cloud Tasks:', taskError);
        // Ne pas faire échouer la création de la planification
      }
    }
